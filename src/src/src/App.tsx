...
export default function App() {
  ...
  // Sintetizador de Sonidos Premium (Web Audio API)
  const playSound = (type: "success" | "error" | "click" | "keyboard" | "birthday") => {
    try {
      ...
      console.warn("Audio Context bloqueado por politicas de navegador.", e);
    }
  };
  ...
  const handleRegisterAsistencia = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    fetch(`${API_URL}?action=registrarAsistencia&dni=${encodeURIComponent(dni)}`)
      ...
      .catch(error => {
        setShowLoader(false);
        setInputDni("");
        presentarErrorPantallyGigante("Error de conexión. Intente nuevamente.");
        console.error(error);
      });
  };
  ...
  const presentarPantallaGigante = (usuario: any, status: string) => {
    ...
    setTimeout(() => {
      setPantallaGigante(prev => {
        if (prev.visible) return { visible: false };
        return prev;
      });
    }, 5000);
  };
  ...
  // Efecto de Confeti (Usa la biblioteca cargada en el index.html)
  ...
  // Crear globos flotantes 3D en pantalla
  const crearGlobosFlotantes = () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      setTimeout(() => {
        ...
        ], { duration: velocidad, easing: "ease-in-out", fill: "forwards" });
      }, retardo);
    }
    setTimeout(() => container.remove(), 7500); 
  };

  // Obtener Datos del Servidor de Google Sheets
  const consultarDatosAdmin = () => {
    ...
    fetch(`${API_URL}?action=getAdminDashboardData`)
      ...
      .then(res => {
        if (!res.success) {
          alert("Error en el panel: " + res.message);
          return;
        }
      ...
      .catch(err => {
        console.error("Error al consultar el panel: ", err);
        setKpis({
          miembrosActivos: "ERR",
          asistenciasHoy: "ERR",
          personasDentro: "ERR",
  ...
  const cargarPlanesEnEditor = () => {
    ...
    fetch(`${API_URL}?action=getPlanes`)
      .then(res => res.json())
      .then(data => {
        if (data.success) setListadoPlanes(data.planes || []);
      })
  ...
  const abrirEditorModal = (cliente: Cliente) => {
    ...
    setEditorModal({ visible: true, cliente });
  ...
  // Recálculo interactivo de fechas de vencimiento de planes
  useEffect(() => {
  ...
  const activarCamaraEditor = () => {
    ...
    setTimeout(() => {
      ...
      .catch(err => {
        alert("Los permisos de cámara están desactivados o bloqueados en este dispositivo.");
  ...
  const handleGuardarCambiosEditor = (e: React.FormEvent) => {
    e.preventDefault();
    setShowLoader(true);
      ...
      if (ctxHours) {
        chartHoursRef.current = new ChartJSClass(ctxHours, {
          ...
          options: {
            ...
            scales: {
              y: { grid: { color: "rgba(30, 41, 59, 0.5)" }, ticks: { color: "#94a3b8", font: { family: "Montserrat", size: 10 } } },
              x: { grid: { display: false }, ticks: { color: "#94a3b8", font: { family: "Montserrat", size: 10 } } }
            },
    ...
    <div className="bg-slate-950 text-slate-100 min-h-screen flex flex-col justify-between font-sans selection:bg-emerald-500 selection:text-slate-950 overflow-x-hidden relative">
      ...
      <main className="flex-grow flex items-center justify-center p-4 lg:p-6 z-10 relative">
        <AnimatePresence mode="wait">
          ...
          {/* ================= REGISTRO DE ASISTENCIA CON LUZ AMBIENTAL TRASERA ================= */}
          {activeTab === "registro" && !pantallaGigante.visible && (
            ...
            >
              {/* EFECTOS DE LUZ AMBIENTAL Y RESPLANDOR TRASERO (GLOWS) */}
              <motion.div 
                animate={{
                  scale: [1, 1.08, 1],
                  opacity: [0.5, 0.8, 0.5],
              ...
              <div className="w-full glass-panel border border-slate-800/80 p-10 md:p-12 rounded-[32px] text-center shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8)] relative overflow-hidden group hover:border-emerald-500/30 transition-all duration-500">
                ...
                <p className="text-[9px] text-slate-500 mt-6 uppercase tracking-[0.2em] font-medium font-display">Digite manualmente su documento</p>
              </div>
            </motion.section>
          )}
                  ...
                  <div>
                    ...
                    <h1 className="text-3xl md:text-4xl lg:text-5xl font-black mt-5 tracking-wide leading-tight text-white capitalize font-display">
                      {pantallaGigante.status === "error" 
                        ? "ERROR DE REGISTRO" 
                        : (pantallaGigante.usuario?.nombre || "DNI NO REGISTRADO")}
                    </h1>
                  </div>
            ...
            >
              <div className="bg-slate-900/40 border border-slate-800/80 rounded-[32px] p-6 md:p-8 shadow-2xl relative overflow-hidden backdrop-blur-2xl h-[86vh] flex flex-col justify-between">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pb-4 border-b border-slate-800/50">
                  <div className="bg-slate-950/80 border border-slate-800 p-4 rounded-2xl flex items-center space-x-3.5 shadow-inner">
                  ...
                  <div className="bg-slate-950/80 border border-slate-800 p-4 rounded-2xl flex items-center space-x-3.5 shadow-inner">
                    ...
                    <div>
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider font-display">Asistencias Hoy</p>
                  ...
                  <div 
                    ...
                    className="bg-slate-950/80 border border-slate-800 p-4 rounded-2xl flex items-center space-x-3.5 shadow-inner cursor-pointer hover:border-amber-500/40 hover:bg-slate-950/90 transition-all duration-300"
                  >
                    <div className="p-3 bg-amber-500/10 text-amber-400 rounded-xl border border-amber-500/20">
                      <Activity className="h-5 w-5" />
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider font-display">Dentro Gym</p>
                      <h3 className="text-xl font-black text-amber-400 font-display">{kpis.personasDentro}</h3>
                    </div>
                  </div>
                  ...
                  <div 
                    ...
                    className="bg-slate-950/80 border border-slate-800 p-4 rounded-2xl flex items-center space-x-3.5 shadow-inner cursor-pointer hover:border-rose-500/40 hover:bg-slate-950/90 transition-all duration-300"
                  >
                    <div className="p-3 bg-rose-500/10 text-rose-400 rounded-xl border border-rose-500/20">
                      <AlertTriangle className="h-5 w-5" />
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider font-display">Por Vencer</p>
                      <h3 className="text-xl font-black text-rose-400 font-display">{kpis.vencidosAlerta}</h3>
                    </div>
                  </div>
                </div>

                {/* ADMIN WORKSPACE (BÚSQUEDA Y SUBSECCIONES) */}
                <div className="flex-grow grid grid-cols-1 lg:grid-cols-12 gap-6 pt-5 overflow-hidden">
                  ...
                  <div className="lg:col-span-4 flex flex-col space-y-4 overflow-hidden">
                    <div className="relative">
                      <input 
                        ...
                        className="w-full bg-slate-950/80 text-white placeholder-slate-700 border border-slate-800/80 focus:border-emerald-500 rounded-2xl pl-11 pr-4 py-3 text-xs font-bold outline-none uppercase tracking-wider transition font-display"
                      />
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button 
                        onClick={() => { playSound("click"); setAdminSubSection("buscador"); }} 
                        className={`py-3.5 rounded-xl font-bold text-[10px] uppercase tracking-wider transition ${
                          adminSubSection === "buscador" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/30" : "bg-slate-950/50 border border-slate-900 text-slate-400 hover:text-white"
                        }`}
                      >
                        Buscador/Ingresos
                      </button>
                      <button 
                        onClick={() => { playSound("click"); setAdminSubSection("graficas"); }} 
                        className={`py-3.5 rounded-xl font-bold text-[10px] uppercase tracking-wider transition ${
                          adminSubSection === "graficas" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/30" : "bg-slate-950/50 border border-slate-900 text-slate-400 hover:text-white"
                        }`}
                      >
                        Estadísticas
                      </button>
                      <button 
                        onClick={() => { playSound("click"); setAdminSubSection("alertas"); }} 
                        className={`py-3.5 rounded-xl font-bold text-[10px] uppercase tracking-wider transition ${
                          adminSubSection === "alertas" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/30" : "bg-slate-950/50 border border-slate-900 text-slate-400 hover:text-white"
                        }`}
                      >
                        Alertas Vencimiento
                      </button>
                      <button 
                        onClick={() => { playSound("click"); setAdminSubSection("inactivos"); }} 
                        className={`py-3.5 rounded-xl font-bold text-[10px] uppercase tracking-wider transition ${
                          adminSubSection === "inactivos" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/30" : "bg-slate-950/50 border border-slate-900 text-slate-400 hover:text-white"
                        }`}
                      >
                        Ausentes
                      </button>
                    ...
                    <AnimatePresence>
                      {showSearchResults && (
                        <motion.div 
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: 10 }}
                          className="flex-grow bg-slate-950/80 border border-slate-800 rounded-2xl p-4 overflow-y-auto space-y-2 max-h-[35vh] lg:max-h-none"
                        >
                          <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest border-b border-slate-800 pb-2">Socios Encontrados</p>
                          {getFilteredClientes().length === 0 ? (
                            <p className="text-xs text-slate-600 text-center py-6 font-display">Ningún socio coincide con la búsqueda</p>
                          ) : (
                            getFilteredClientes().map(c => (
                              <div 
                                key={c.dni} 
                                onClick={() => abrirEditorModal(c)}
                                className="p-3 bg-slate-900 hover:bg-emerald-500/10 border border-slate-800 rounded-xl flex justify-between items-center cursor-pointer transition"
                              >
                                <div>
                                  <p className="text-xs font-bold text-white capitalize">{c.nombre}</p>
                                  <p className="text-[9px] text-slate-500 font-mono mt-0.5">{c.plan} • DNI {c.dni}</p>
                                </div>
                                <span className={`text-[8px] font-bold px-2 py-1 rounded-full uppercase ${
                                  c.estado === "Activo" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-rose-500/10 text-rose-400 border border-rose-500/20"
                                }`}>
                                  {c.estado}
                                </span>
                              </div>
                  ...
                  {/* Right: Sub-section Content Container */}
                  <div className="lg:col-span-8 bg-slate-950/50 border border-slate-850 rounded-2xl p-5 overflow-y-auto max-h-[50vh] lg:max-h-none">
                    
                    {/* SUBSECCIÓN 1: BUSCADOR / INGRESO EN TIEMPO REAL */}
                    {adminSubSection === "buscador" && (
                      <motion.div 
                        ...
                        className="space-y-4"
                      >
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest font-display flex items-center gap-1.5">
                          <Activity className="h-3.5 w-3.5 text-emerald-400" />
                          Últimos Ingresos Registrados Hoy
                        </h4>
                        <div className="space-y-2">
                          {ultimosIngresos.length === 0 ? (
                            <div className="text-center text-slate-600 py-12 uppercase tracking-widest text-xs font-display">
                              Aún no hay registros de asistencia de socios hoy
                            </div>
                          ) : (
                            ultimosIngresos.map((item, idx) => (
                              <div 
                                key={idx} 
                                className="bg-slate-900/40 hover:bg-slate-900/60 p-3 rounded-xl border border-slate-800/40 flex justify-between items-center transition"
                              >
                                <div className="flex items-center space-x-3">
                                  <div className="w-8 h-8 rounded-full overflow-hidden bg-slate-950 border border-slate-800 flex items-center justify-center">
                                    <img 
                                  ...
                                  <div>
                                    <h5 className="text-xs font-bold text-white capitalize">{item.nombre}</h5>
                                    <p className="text-[9px] text-slate-500 uppercase font-bold tracking-wider font-display">
                                      {item.plan} • DNI <span className="font-mono">{item.dni}</span>
                                    </p>
                                  </div>
                                </div>
                                <div className="text-right space-y-1">
                                  ...
                                  <span className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider ${
                                    item.movimiento === "IN" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-orange-500/10 text-orange-400 border border-orange-500/20"
                                  }`}>
                                    {item.movimiento || "IN"}
                                  </span>
                                </div>
                    ...
                    {/* SUBSECCIÓN 2: GRÁFICAS OPERATIVAS */}
                    {adminSubSection === "graficas" && (
                      ...
                      >
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          <div className="bg-slate-900/30 p-5 rounded-2xl border border-slate-800/80 shadow-md">
                            <h4 className="text-[10px] font-black text-slate-400 mb-4 uppercase tracking-widest font-display">
                              Horas Pico de Ingreso
                            </h4>
                            <div className="relative h-48 w-full">
                              <canvas id="chartHoursCanvas" className="w-full h-full"></canvas>
                            </div>
                          </div>

                          <div className="bg-slate-900/30 p-5 rounded-2xl border border-slate-800/80 shadow-md">
                            <h4 className="text-[10px] font-black text-slate-400 mb-4 uppercase tracking-widest font-display">
                              Flujo de Asistencias por Día
                            </h4>
                            <div className="relative h-48 w-full">
                              <canvas id="chartDaysCanvas" className="w-full h-full"></canvas>
                            </div>
                          </div>
                        </div>
                      </motion.div>
                    ...
                    {/* SUBSECCIÓN 3: MEMBRESÍAS POR VENCER */}
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {proximosVencer.length === 0 ? (
                            <div className="col-span-2 text-center text-slate-600 py-8 uppercase tracking-widest text-xs font-display">
                              No hay avisos de vencimientos urgentes
                            </div>
                          ) : (
                            proximosVencer.map((alertItem, idx) => (
                              <div 
                                key={idx} 
                                className="bg-slate-900/40 hover:bg-slate-900/60 border-l-[4px] border-amber-500 p-4 rounded-r-2xl flex justify-between items-center transition duration-200 group border-y border-r border-slate-800/40"
                              >
                                <div className="space-y-1 max-w-[80%]">
                                  <h5 className="text-xs font-black text-white capitalize truncate">{alertItem.nombre}</h5>
                                  <p className="text-[9px] text-slate-500 uppercase font-bold tracking-wider font-display">
                                    {alertItem.plan} • Vence: <span className="text-amber-400 font-mono">{alertItem.vencimiento}</span>
                                  </p>
                                </div>
                                <span className="text-amber-500 p-2 rounded-xl bg-amber-500/5 border border-amber-500/10 group-hover:scale-110 transition duration-300">
                    ...
                    {/* SUBSECCIÓN 4: CLIENTES INACTIVOS */}
                    {adminSubSection === "inactivos" && (
                      ...
                      >
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 font-display">
                          ...
                          Ausentes (&gt;7 días de inactividad)
                        </h4>
                        
                        <div className="overflow-x-auto">
                          <table className="w-full text-left border-collapse text-xs">
                            <thead>
                              <tr className="border-b border-slate-800 text-slate-500 font-bold uppercase tracking-wider font-display">
                                <th className="pb-2">Ausencia</th>
                            ...
                            <tbody className="divide-y divide-slate-900 text-slate-300">
                              {inactivosList.length === 0 ? (
                                <tr>
                                  <td colSpan={4} className="py-8 text-center text-slate-600 uppercase tracking-widest font-display">
                                    No hay socios inactivos registrados.
                                  </td>
                                </tr>
                              ) : (
                                inactivosList.map((c, idx) => {
                                  const waLink = `https://wa.me/51${c.celular}?text=Hola%20${encodeURIComponent(c.nombre)}%2C%20te%20escribimos%20de%20Fitness%20Club.%20Notamos%20que%20no%20nos%20visitas%20hace%20${c.diasInactivo}%20d%C3%ADas.%20%C2%A1Te%20esperamos%20pronto%20para%20seguir%20entrenando!`;
                                  return (
                                    <tr key={idx} className="hover:bg-slate-900/30 transition duration-150">
                                      <td className="py-3 font-extrabold text-rose-400 text-xs uppercase tracking-widest font-display">
                                        {c.diasInactivo} días
                                      </td>
                                      <td className="py-3 font-extrabold text-white capitalize">{c.nombre}</td>
                                      <td className="py-3 font-semibold text-slate-500 font-mono">{c.ultimoRegistro}</td>
                                      <td className="py-3 text-right">
                                        <a 
                                          ...
                                          className="inline-flex items-center justify-center bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black px-3.5 py-2 rounded-xl text-[9px] uppercase tracking-wider transition gap-1.5 shadow-md active:scale-95 cursor-pointer"
                                        >
        ...
        {editorModal.visible && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <motion.div 
              ...
              className="bg-slate-900 border border-slate-800 w-full max-w-4xl rounded-[32px] overflow-hidden shadow-2xl flex flex-col md:flex-row max-h-[90vh] md:max-h-none overflow-y-auto md:overflow-y-visible relative"
            >
              <div className="absolute top-4 right-4 z-10 md:hidden">
                <button 
                  onClick={cerrarEditorModal} 
                  className="bg-slate-950 text-slate-400 hover:text-white p-2 rounded-full border border-slate-800 cursor-pointer"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>

              {/* Columna Izquierda: Portrait & Cámara */}
              <div className="w-full md:w-2/5 bg-slate-950 p-6 flex flex-col items-center justify-between border-b md:border-b-0 md:border-r border-slate-800/80 gap-5">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest font-display">Foto del Socio</h3>
                <div className="w-44 h-44 rounded-3xl overflow-hidden border-4 border-slate-800 bg-slate-900 shadow-inner flex items-center justify-center relative">
                  {cameraActive ? (
                ...
                <div className="w-full space-y-2">
                  {!cameraActive && !fotoBase64Editor && (
                    <button 
                      type="button" 
                      onClick={activarCamaraEditor} 
                      className="w-full bg-slate-850 hover:bg-slate-800 text-white text-xs font-bold py-3 rounded-xl uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-2 border border-slate-800/80 cursor-pointer"
                    >
                  ...
                  {cameraActive && (
                    <button 
                      ...
                      className="w-full bg-emerald-500 hover:bg-emerald-600 text-slate-950 text-xs font-black py-3 rounded-xl uppercase tracking-widest transition-all duration-300 shadow-lg cursor-pointer"
                    >
                      Capturar Foto
                    </button>
                  )}
                  {fotoBase64Editor && (
                    <button 
                      ...
                      className="w-full bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs font-bold py-3 rounded-xl uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      Descartar Foto
                    </button>
                  )}
              ...
              {/* Columna Derecha: Formulario del Socio */}
              <div className="w-full md:w-3/5 p-6 md:p-8 space-y-5">
                <div className="hidden md:flex justify-between items-center border-b border-slate-800 pb-3">
                  <h3 className="text-sm font-black text-white uppercase tracking-widest font-display">Modificar Socio</h3>
                  <button onClick={cerrarEditorModal} className="text-slate-500 hover:text-white transition duration-200 cursor-pointer">
                    <X className="h-5 w-5" />
                  </button>
                </div>

                <form onSubmit={handleGuardarCambiosEditor} className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-display">DNI (No editable)</label>
                    <input type="text" value={editFormData.dni} readOnly className="w-full bg-slate-950 text-slate-500 border border-slate-800/80 rounded-xl p-3 text-xs font-bold outline-none font-mono" />
                  </div>

                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-display">Nombre Completo</label>
                    <input type="text" value={editFormData.nombre} onChange={(e) => handleEditorFormChange("nombre", e.target.value)} required className="w-full bg-slate-950 text-white border border-slate-800 focus:border-emerald-500 rounded-xl p-3 text-xs font-bold outline-none capitalize" />
                  </div>

                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-display">F. de Nacimiento</label>
                    <input type="text" value={editFormData.fechaNacimiento} onChange={(e) => handleEditorFormChange("fechaNacimiento", e.target.value)} placeholder="DD/MM/AAAA" required className="w-full bg-slate-950 text-white border border-slate-800 focus:border-emerald-500 rounded-xl p-3 text-xs font-bold outline-none font-mono" />
                  </div>

                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-display">Número Celular</label>
                    <input 
                      type="text" 
                      value={editFormData.celular} 
                      maxLength={9}
                      inputMode="numeric"
                      onChange={(e) => handleEditorFormChange("celular", e.target.value.replace(/[^0-9]/g, ''))}
                      required 
                      className="w-full bg-slate-950 text-white border border-slate-800 focus:border-emerald-500 rounded-xl p-3 text-xs font-bold outline-none font-mono" 
                    />
                  </div>

                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-display">Plan Contratado</label>
                    <select value={editFormData.plan} onChange={(e) => handleEditorFormChange("plan", e.target.value)} className="w-full bg-slate-950 text-white border border-slate-800 rounded-xl p-3 text-xs font-bold outline-none cursor-pointer uppercase">
                      {listadoPlanes.map((p) => (
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-display">Estado de Membresía</label>
                    <select value={editFormData.estado} onChange={(e) => handleEditorFormChange("estado", e.target.value)} className="w-full bg-slate-950 text-white border border-slate-800 rounded-xl p-3 text-xs font-bold outline-none cursor-pointer uppercase">
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-display">Fecha de Inicio</label>
                    <input type="text" value={editFormData.fechaInicio} onChange={(e) => handleEditorFormChange("fechaInicio", e.target.value)} placeholder="DD/MM/AAAA" required className="w-full bg-slate-950 text-white border border-slate-800 focus:border-emerald-500 rounded-xl p-3 text-xs font-bold outline-none font-mono" />
                  </div>

                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 font-display">Vencimiento</label>
                    <input type="text" value={editFormData.fechaVencimiento} readOnly placeholder="AUTOMÁTICO" required className="w-full bg-slate-900 text-emerald-400 border border-slate-800 rounded-xl p-3 text-xs font-black text-center outline-none font-mono shadow-inner" />
                  </div>

                  <button type="submit" className="col-span-2 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-slate-950 font-black py-3.5 rounded-xl text-[10px] uppercase tracking-widest transition duration-300 shadow-lg hover:shadow-[0_4px_15px_rgba(16,185,129,0.3)] mt-2 flex items-center justify-center gap-2 active:scale-95 cursor-pointer">
                    <Save className="h-4 w-4" />
                    Guardar Cambios de Miembro
                  </button>
                </form>
              </div>
            </motion.div>
      ...
      {/* ================= MODAL: LISTADO "DENTRO GYM" ================= */}
      <AnimatePresence>
        {showDentroGymModal && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-[28px] overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
              <div className="flex justify-between items-center border-b border-slate-800/80 px-6 py-4">
                ...
                <button onClick={() => { playSound("click"); setShowDentroGymModal(false); }} className="text-slate-500 hover:text-white transition duration-150 cursor-pointer">
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="overflow-y-auto px-4 py-3 space-y-2 max-h-[60vh]">
                {personasDentroList.length === 0 ? (
                  <p className="text-xs text-center text-slate-500 py-8 uppercase tracking-widest font-display">No hay nadie dentro del gym</p>
                ) : (
                  personasDentroList.map((persona, index) => (
                    <div 
                      key={index}
                      onClick={() => { playSound("click"); setFotoDentroModal({ visible: true, persona }); }}
                      className="flex justify-between items-center p-3 rounded-xl cursor-pointer transition border border-slate-800/50 bg-slate-950/60 hover:bg-emerald-500/5 hover:border-emerald-500/20 group"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-slate-900 border border-slate-800 overflow-hidden flex items-center justify-center">
                          <img 
                            src={persona.foto || DEFAULT_AVATAR} 
                            alt={persona.nombre}
                            className="w-full h-full object-cover"
                            onError={(e) => { (e.target as HTMLImageElement).src = DEFAULT_AVATAR; }}
                          />
                        ...
                        <div>
                          <p className="text-xs font-bold text-white capitalize group-hover:text-emerald-400 transition">{persona.nombre}</p>
                          <p className="text-[9px] text-slate-500 uppercase font-medium tracking-wider mt-0.5 font-mono">Ingresó: {persona.horaIngreso || "--:--"}</p>
                        </div>
                      </div>
                      <span className="px-2.5 py-1 rounded-full text-[9px] font-black tracking-widest bg-amber-500/10 text-amber-400 border border-amber-500/20 flex items-center gap-1 font-mono">
                        <Hourglass className="h-3 w-3 animate-spin" style={{ animationDuration: "3s" }} />
                        {formatMinutesInside(persona.minutosDentro)}
                      </span>
                    </div>
                  ))
                )}
      ...
      {/* ================= MODAL SOCIO DETALLE CON MARCO NEÓN VERDE RESPIRATORIO ================= */}
      <AnimatePresence>
        {fotoDentroModal.visible && (
          <div className="fixed inset-0 bg-slate-950/92 backdrop-blur-md z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative flex flex-col items-center gap-6 max-w-sm w-full">
              <button onClick={() => { playSound("click"); setFotoDentroModal({ visible: false }); }} className="absolute -top-4 -right-4 text-slate-400 hover:text-white text-lg bg-slate-900 rounded-full w-10 h-10 flex items-center justify-center border border-slate-800 shadow-xl cursor-pointer">
                <X className="h-5 w-5" />
              </button>

              <div className="neon-green-frame w-56 h-56 md:w-64 md:h-64">
                <img 
                  src={fotoDentroModal.persona?.foto || DEFAULT_AVATAR} 
                  alt="Socio" 
                  className="w-48 h-48 md:w-56 md:h-56 object-cover border-4 border-emerald-400 rounded-full shadow-2xl relative z-10" 
                  onError={(e) => { (e.target as HTMLImageElement).src = DEFAULT_AVATAR; }}
                />
              </div>

              <div className="text-center space-y-1.5 relative z-10">
                <h4 className="text-xl md:text-2xl font-black text-white capitalize font-display">{fotoDentroModal.persona?.nombre}</h4>
                <p className="text-xs font-extrabold text-emerald-400 uppercase tracking-widest bg-emerald-500/10 border border-emerald-500/20 px-4 py-1.5 rounded-full inline-flex items-center gap-1.5 font-display">
                  Dentro del gym • {formatMinutesInside(fotoDentroModal.persona?.minutosDentro || 0)}
                </p>
              </div>
            </motion.div>
          </div>
      ...
      <AnimatePresence>
        {showLoader && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-950/95 flex flex-col items-center justify-center z-50 backdrop-blur-sm">
            ...
            <p className="text-emerald-400 text-xs uppercase tracking-[0.35em] font-black animate-pulse font-display">{loaderMessage}</p>
          </motion.div>
        )}
      </AnimatePresence>

      <footer className="bg-slate-900/40 border-t border-slate-800/40 px-6 py-4 text-center text-[10px] text-slate-500 font-medium tracking-wide font-display">
        &copy; 2026 Fitness Club Control Hub. Todos los derechos reservados.
      </footer>
    </div>
  );
}
